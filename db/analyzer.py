...
class MarketDB:
    def __init__(self):
        ...
        self.conn = pymysql.connect(host='localhost', user='root', port=3306,
                               ...
                               db=os.environ['MY_DB_NAME'], charset='utf8')
        self.engine = create_engine('mysql+pymysql://root:{}@{}:{}/{}'.format(
            os.environ['MY_DB_PASSWORD'], 'localhost', '3306', os.environ['MY_DB_NAME']))
    ...
    def get_daily_price(self, code, start_date=None, end_date=None):
        """
            KRX 종목의 일별 시세를 DataFrame 형태로 변환
        ...
        if (start_date is None):
            ...
            df = pd.read_sql(sql, self.engine.connect())
            df.index = df['date']
            return df